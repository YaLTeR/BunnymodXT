...
HOOK_DEF_1(HwDLL, int, __cdecl, Host_FilterTime, float, passedTime)
{
	...
	if (pHost_FilterTime_FPS_Cap_Byte)
	{
		if (*reinterpret_cast<byte*>(pHost_FilterTime_FPS_Cap_Byte) == 0x7E) {
			is_0x7E = true;
			is_0x7E_on_init = true;
		} else if (*reinterpret_cast<byte*>(pHost_FilterTime_FPS_Cap_Byte) == 0x75) {
			is_0x75 = true;
		...
		if (is_0xEB && ((is_0x7E_on_init && !CVars::sv_cheats.GetBool()) || !CVars::bxt_remove_fps_limit.GetBool()))
		{
			if (is_0x7E_on_init)
...
HOOK_DEF_4(HwDLL, void, __cdecl, SPR_Set, HSPRITE_HL, hSprite, int, r, int, g, int, b)
{
	...
	if (cl.custom_hud_color_set && !cl.bxt_hud_color_set && !insideDrawCrosshair && !cl.insideDrawAmmoHistory && !cl.insideDrawHealthDamage && !cl.insideDrawHealthPain && !cl.insideDrawNightVision)
	{
		r = cl.custom_r;
		g = cl.custom_g;
		b = cl.custom_b;
