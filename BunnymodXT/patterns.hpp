...
namespace patterns
{
	namespace engine
	{
		...
		PATTERNS(NLoadBlobFileClient,
			"HL-NGHL",
			"8B 0D ?? ?? ?? ?? 8B 54 24 ?? 53 56",
			"HL-4554",
			"B8 00 10 00 00 E8 ?? ?? ?? ?? 8B 84 24 ?? ?? ?? ?? 53 33 DB"
		);
		...
		PATTERNS(R_LoadSkys,
			"HL-SteamPipe",
			"55 8B EC 83 EC 6C A1 ?? ?? ?? ?? 56 57 33 FF 3B C7 89 7D ?? 75 ?? BE ?? ?? ?? ?? 39 3E",
			"HL-4554",
			"83 EC 6C A1 ?? ?? ?? ?? 56 85 C0 C7 44 24 ?? 00 00 00 00 75 ?? 33 F6",
			"HL-WON-1712",
			"83 EC 6C A1 ?? ?? ?? ?? 56 33 F6 3B C6 89 74 24 ?? 75",
			"CoF-5936",
		...
		PATTERNS(PF_changelevel_I,
			"HL-Steampipe",
			"55 8B EC A1 ?? ?? ?? ?? 8B 0D ?? ?? ?? ?? 3B C1 74 ?? A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 45 ?? 85 C0",
			"HL-4554",
			"A1 ?? ?? ?? ?? 8B 0D ?? ?? ?? ?? 3B C1 74 ?? A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 44 24 ?? 85 C0",
			"CoF-5936",
